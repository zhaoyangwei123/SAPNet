...
class SOLOV2Head(SOLOHead):
    ...
    def forward(self, feats):
        ...
        affinity_feats = [feat.float() for feat in feats]
        new_feats = self.split_feats(affinity_feats)
        ...
        feature_pred = mask_feats
        N, c, h, w = feature_pred.shape
        feature_pred = feature_pred.view(-1, h, w).unsqueeze(0)
        ins_pred = []
        eval = True
        for i in range(5):
            kernel = mlvl_kernel_preds[i].permute(0, 2, 3, 1).contiguous().view(-1, c).unsqueeze(-1).unsqueeze(-1)
            ins_i = F.conv2d(feature_pred, kernel, groups=N).view(N, self.num_grids[i] ** 2, h, w)
            h_1,w_1 = featmap_sizes[i][0] * 2, featmap_sizes[i][1] * 2
            if not eval:
                ins_i = F.interpolate(ins_i, size=(h_1,w_1), mode='bilinear')
            if eval:
                ins_i = ins_i.sigmoid()
            ins_pred.append(ins_i)

        return mlvl_kernel_preds, mlvl_cls_preds, mask_feats, ins_pred
    ...
    def get_results(self, img, mlvl_kernel_preds, mlvl_cls_scores, mask_feats, ins_preds,
                    img_metas, **kwargs):
        ...
        return result_list
    def apro_output(self, output, img):
            h,w= output.size()[-2:]
            img = F.interpolate(img, size=(h, w),mode='bilinear', align_corners=False)
            tree = self.mst(img)
            AS_1= self.global_apro(feature_in=output.float(),embed_in=img,tree=tree, zeta_g=0.01)
            # AS_2= self.global_apro(AS_1,embed_in=AS_1,tree=tree, zeta_g=0.1)
            AS_2=self.local_apro(img, AS_1)
            # AS =(AS 1 +AS 2)/2.0
            return AS_2
    def _get_results_single(self,
        ...
        """
        # def apro_output(self, output, img):
        #     batch,_,h,w= output.size()
        #     img = F.interpolate(img, size=(h, w),mode='bilinear', align_corners=False)
        #     tree = self.mst(img)
        #     AS_1= self.global_apro(feature_in=output.float(),embed_in=img,tree=tree, zeta_g=0.1)
        #     AS_2=self.local_apro(img, AS_1)
        #     # AS =(AS 1 +AS 2)/2.0
        #     return AS_2
        ...
        upsampled_size = (featmap_size[0] * self.mask_stride,
                          featmap_size[1] * self.mask_stride)
        # mask_feats = self.apro_output(mask_feats, img)
        # process.
        ...
        # mask_preds = self.apro_output(mask_preds, img).squeeze(0)
        # mask.
        ...
        # masks = self.apro_output(masks, img).squeeze(0)
        results.masks = masks
    ...
    def get_seg(self, img, mlvl_kernel_preds, mlvl_cls_scores, mask_feats, seg_preds,  img_metas,  **kwargs):
        cate_preds = mlvl_cls_scores
        ...
        for img_id in range(len(img_metas)):
            cate_pred_list = [
                cate_preds[i][img_id].view(-1, self.cls_out_channels).detach() for i in range(num_levels)
            ]
            seg_pred_list = [
