...
class BaseMaskHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self,
        ...
        loss1 = self.loss(
            *outs,
            epoch,
            gt_labels=gt_labels,
            gt_masks = [x[1] for x in mask_list],
            img = img,
            img_metas=img_metas,
            gt_bboxes=gt_bboxes,
            gt_bboxes_ignore=gt_bboxes_ignore,
            positive_infos=positive_infos,
            **kwargs)     
        loss_mask += loss1['loss_mask'] 
        loss3 = self.loss(
            *outs,
            epoch,
            gt_labels=gt_labels,
            gt_masks = [x[3] for x in mask_list],
            img = img,
            img_metas=img_metas,
            gt_bboxes=gt_bboxes,
            gt_bboxes_ignore=gt_bboxes_ignore,
            positive_infos=positive_infos,
            **kwargs)     
        loss_mask += loss3['loss_mask']
        loss5 = self.loss(
            *outs,
            epoch,
            gt_labels=gt_labels,
            gt_masks = [x[5] for x in mask_list],  #5->3 hqsam
            img = img,
            img_metas=img_metas,
            gt_bboxes=gt_bboxes,
            gt_bboxes_ignore=gt_bboxes_ignore,
            positive_infos=positive_infos,
            **kwargs)     
        loss_mask += loss5['loss_mask']
        loss['loss_mask'] = 0.5*loss_mask/6 + loss['loss_mask']
        return loss
    ...
    def simple_test(self,
        ...
        results_list = self.get_results(img,
            *mask_inputs,
            rescale=rescale,
            instances_list=instances_list,
            **kwargs)
        # results_list = self.get_seg(img,
        #     *mask_inputs,
        #     rescale=rescale,
        #     instances_list=instances_list,
        #     **kwargs)
