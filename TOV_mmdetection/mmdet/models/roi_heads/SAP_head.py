...
class SAPHead(StandardRoIHead):
    """Simplest base roi head including one bbox head and one mask head. top由7到1,只从3个里面取一个"""

    def __init__(self, bbox_roi_extractor, num_stages, bbox_head, top_k=2, with_atten=None, **kwargs):
        super(SAPHead, self).__init__(bbox_roi_extractor=bbox_roi_extractor, bbox_head=bbox_head, **kwargs)
        self.threshold = 0.3
        ...
        self.topk1 = top_k  # 7->1->2merge
        self.topk2 = top_k  # 7->1->2merge
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, proposal_list_base, proposals_list, proposals_valid_list,
                                                    ...
                                                    img_metas, stage,epoch)

            losses.update(bbox_results['loss_instance_mil'])
        return losses, bbox_results['pseudo_boxes'], bbox_results['dynamic_weight'], bbox_results['pro_scores'], bbox_results['proposals_pro'] #gai by wzy
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        bbox_results = self._bbox_forward(x, rois, gt_points, stage, proposals_list)
        ...
        if neg_proposal_list is not None:
            ...
            neg_bbox_results = self._bbox_forward(x, neg_rois, None, stage, None)  ######stage
        ...
        pseudo_boxes, mean_ious, filtered_boxes, filtered_scores, dynamic_weight, pro_scores = self.merge_box(bbox_results,
        ...
        bbox_results.update(pro_scores=dynamic_weight) #add by wzy
        def generate_coordinates(point_x, point_y, w_pro, h_pro, n_values, multiplier):
            a = ((point_x - w_pro * (1 + multiplier * n_values)) / 2).to(proposals)
            b = ((point_y - h_pro * (1 + multiplier * n_values)) / 2).to(proposals)
            c = ((point_x + w_pro * (1 + multiplier * n_values)) / 2).to(proposals)
            d = ((point_y + h_pro * (1 + multiplier * n_values)) / 2).to(proposals)
            return torch.stack((a, b, c, d)).T
        if stage == 0:
            cls_scores = bbox_results['cls_score']
            batch_gt = [len(b) for b in gt_points]
            cls_scores = torch.split(cls_scores, batch_gt)
            labels = torch.split(gt_labels, batch_gt)
            L_scores = list()
            points = [bbox_xyxy_to_cxcywh(b)[:, :2] for b in gt_points]
            proposals_pro = list()
            for m in range(len(proposals_list)):
                L_scores.append(torch.ones(cls_scores[m].shape[0], cls_scores[m].shape[1]).cuda())
                proposals = proposals_list[m].view(len(gt_points[m]), -1, proposals_list[m].shape[-1])
                proposals_pro.append(torch.ones(proposals.shape[0], 26, 4).to(proposals))  
                for i in range(len(gt_points[m])):
                    sum=0
                    for j in range(6):
                        for k in range(len(points[m])):
                            if (proposals[i][j][0] < points[m][k][0] <proposals[i][j][2]) and (proposals[i][j][1] < points[m][k][1] < proposals[i][j][3]) and labels[m][i]==labels[m][k] and k!= i:
                                point_x = (points[m][i][0]-points[m][k][0])**2
                                point_y = (points[m][i][1]-points[m][k][1])**2
                                L_scores[m][i][j] += (point_x + point_y).pow(0.25)
                        L_scores[m][i][j] = (1/(L_scores[m][i][j]-1)).sigmoid()
                        proposals_pro[m][i][j] = proposals[i][j]
                        if  ((proposals[i][j][0]-1 < pseudo_boxes[m][i][0] < proposals[i][j][0]+1) and (proposals[i][j][1]-1 < pseudo_boxes[m][i][1] < proposals[i][j][1]+1)  and (proposals[i][j][2]-1 < pseudo_boxes[m][i][2] < proposals[i][j][2]+1)  and (proposals[i][j][3]-1 < pseudo_boxes[m][i][3] < proposals[i][j][3]+1)  and sum==0) or(j==5 and sum==0) :
                            sum +=1
                            w_pro, h_pro = proposals[i][j][2] - proposals[i][j][0], proposals[i][j][3] - proposals[i][j][1]
                            point_x, point_y = proposals[i][j][2] + proposals[i][j][0], proposals[i][j][3] + proposals[i][j][1]
                            n_values = torch.arange(1, 16).to(proposals)
                            e = generate_coordinates(point_x, point_y, w_pro, h_pro, n_values, 0.01)
                            proposals_pro[m][i][6:21] = e[0:15]
                            if L_scores[m][i][j] == 1:
                                n_values = torch.arange(16, 21).to(proposals)
                                e = generate_coordinates(point_x, point_y, w_pro, h_pro, n_values, 0.005)
                                proposals_pro[m][i][21:26] = e[0:5]
                            else:
                                n_values = torch.arange(1, 6).to(proposals)   
                                e = generate_coordinates(point_x, point_y, w_pro, h_pro, n_values, -0.005 / L_scores[m][i][j])
                                proposals_pro[m][i][21:26] = e[0:5]
                proposals_pro[m]=proposals_pro[m].reshape(-1,4)
                proposals_pro[m]=proposals_pro[m].clamp(min=0)
            cls_scores, L_scores = torch.cat(cls_scores), torch.cat(L_scores)
            L_scores = L_scores.pow(0.0625 * 0.25)
            L_scores = L_scores.unsqueeze(2).repeat(1, 1, cls_scores.shape[-1])
            cls_scores = cls_scores * L_scores
            bbox_results['cls_score'] = cls_scores
            bbox_results['proposals_pro'] = proposals_pro#gai by wzy
        else:
            bbox_results['proposals_pro'] = proposals_list
    ...
    def _bbox_forward(self, x, rois, gt_points, stage, proposals_list):
    ...
    def merge_box(self, bbox_results, proposals_list, proposals_valid_list, gt_labels, gt_bboxes, img_metas, stage,epoch):
        cls_scores = bbox_results['cls_score']
        ins_scores = bbox_results['ins_score']
        num_instances = bbox_results['num_instance']
        if stage < 3:
            cls_scores = cls_scores.softmax(dim=-1)      #adapt to sigmoid increase the gap between proposals
        else:
        ...
        pseudo_bboxes = torch.cat(boxes).detach()
        pseudo_boxes = torch.split(pseudo_bboxes, batch_gt)

        for i in range(len(gt_bboxes)):        #add by wzy
            num_gt = len(gt_labels[i])
            proposals = proposals_list[i].view(num_gt, -1, 4)
            for j in range(num_gt):
                pseudo_box = pseudo_boxes[i][j]
                w_box = pseudo_box[2]-pseudo_box[0]
                h_box =  pseudo_box[3]-pseudo_box[1]
                iou_1 = 0
                for k in range(3):
                    w = proposals[j][k][2]-proposals[j][k][0]
                    h = proposals[j][k][3]-proposals[j][k][1]
                    if w_box*h_box < w*h:
                        iou_2 = bbox_overlaps(pseudo_box.unsqueeze(0), proposals[j][k].unsqueeze(0), is_aligned=True)
                        if iou_2 > 0.6 and iou_2>iou_1:   #0.7->0.5
                            iou_1 = iou_2
                            pseudo_boxes[i][j] = (proposals[j][k]+ (pseudo_boxes[i][j]*iou_2))/(iou_2+1)  #iou weight
                            w_box, h_box = w, h
                            k = 0
                        elif iou_2 > 0.3 and (proposals[j][k][0]-2)<pseudo_box[0] and (proposals[j][k][1]-2)<pseudo_box[1] and (proposals[j][k][2]+2)>pseudo_box[2] and (proposals[j][k][3]+2)>pseudo_box[3] and iou_2>iou_1:  #2->5
                            iou_1 = iou_2
                            pseudo_boxes[i][j] = ((proposals[j][k]*iou_2) + pseudo_boxes[i][j])/(iou_2+1)
                            w_box, h_box = w, h
                            k = 0
                        else:
                            continue
        ...
        iou1 = bbox_overlaps(pseudo_bboxes, torch.cat(gt_bboxes), is_aligned=True)
        ...
        if self.test_mean_iou and stage == 1:
            self.sum_iou = self.sum_iou+iou1.sum()
            self.sum_num = self.sum_iou+len(iou1)
            print('\r', self.sum_iou / self.sum_num, end='', flush=True)
        

        return list(pseudo_boxes), mean_ious, list(filtered_boxes), list(filtered_scores), dynamic_weight.detach(), cls_score.detach()
