...
class MILLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 # reduction='mean',
                 binary_ins=False,
                 loss_weight=1.0, eps=1e-6, loss_type='gfocal_loss', L=512, D=128, K=1):
        """
        Args:
        ...
        #self.attention = Attention_Gated(L, D, K)  #add by wzy
    ...
    def forward(self, bag_cls_prob, bag_ins_outs, labels, valid, weight=None):
        ...
        #prob = (prob_cls * prob_ins).max(dim=1)[0]
        #prob = (prob_cls * prob_ins).mean(dim=1)
